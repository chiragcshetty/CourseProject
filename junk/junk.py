...
class Corpus(object):
    ...
    def expectation_step(self):
        ...
        temp_sum_on_topics = temp_Z.sum(axis=1)

        self.topic_prob = temp_Z/temp_Z.sum(axis=1)[:,np.newaxis,:] #hidden variable for topic choice for each word in each doc (M*N)
        ##################

        temp_B = np.repeat(self.background_prob[np.newaxis,:], self.number_of_documents, axis=0)
        
        temp_numerator = self.lambda_B*temp_B
        temp_denominator = (1-self.lambda_B)*temp_sum_on_topics + temp_numerator

        self.B_prob = temp_numerator/temp_denominator  ##hidden variable for choice from background model for each word in each doc (M*N)
        ##################
    ...
    def maximization_step(self, number_of_topics):
        ...
        temp_B = np.repeat(self.B_prob[:, np.newaxis, :], number_of_topics, axis=1)
        temp = np.multiply(temp_C,self.topic_prob)
        temp1  = np.multiply(temp,temp_B)
    ...
    def calculate_likelihood(self, number_of_topics):
        ...
        temp_netP = np.log(temp_Z.sum(axis=1)) # net prob of a given word in given doc p(w/d) = sum over all z, p(w/d,z)*p(z)
        
        temp = np.multiply(self.term_doc_matrix, self.lambda_B*temp_B + (1-self.lambda_B)*temp_netP )
...
def main():
    ...
    N = 8 #no of docs
    name_set=['elon']
    corpus = Corpus(documents_path, N, name_set)  # instantiate corpus
    corpus.build_corpus()
    corpus.build_vocabulary()
    corpus.build_background_prob()
    #print(corpus.vocabulary)
    #print(corpus.background_prob)
    ...
    for topic_no in range(number_of_topics):
        ...
        idx = sorted(range(len(a)), key=lambda i: a[i], reverse=True)[:20]
...
if __name__ == '__main__':
    ...
    # for topic_no in range(number_of_topics):
    #     a = corpus.topic_word_prob[topic_no,:]
    #     idx = sorted(range(len(a)), key=lambda i: a[i], reverse=True)[:10]
    #     print([corpus.vocabulary[j] for j in idx])
